...
SERVER_NAME = os.getenv("GANTRY_SERVER_NAME", "Gantry")
...
def require_auth(req: Request, request_id: str, action_config: dict = None) -> None:
    ip = client_ip(req)
    ...
    # If no action config provided (e.g., the /actions endpoint), default to global
    if action_config is None:
        action_config = {}

    # Check for route-specific token rules (Default is True -> use global)
    req_token_val = action_config.get("require_token", True)

    # RULE A: Explicitly Public Route
    if req_token_val is False:
        return  # Skip token check entirely for this route

    # RULE B: Custom Route Token
    if isinstance(req_token_val, str) and req_token_val.strip():
        expected_token = req_token_val.strip()
    # RULE C: Fallback to Global API Token
    else:
        expected_token = API_TOKEN

    # Perform the validation
    if expected_token:
        provided_token = req.headers.get("X-Token", "")
...
def check_rate_limit(action_name: str, action_config: dict, ip: str, request_id: str) -> None:
    ...
    if len(RATE_LIMIT_STORE[action_name]) > 1000:
        RATE_LIMIT_STORE[action_name].clear()
        history = [] 
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            if response.status_code != 429:
                log_func = logger.debug if path == "/health" else logger.info
                log_func(f"{request_id} | REQ | {method} {path} | {response.status_code} | {ms}ms | ip={ip}")
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    # We ALWAYS log the full detailed error to the server file/console
    logger.error(f"{rid} | UNHANDLED 500 | {request.method} {request.url.path} | err={type(exc).__name__}: {exc}", exc_info=True)
    
    # But we only show the detail to the user if the log level is set to DEBUG
    # If it's INFO, WARNING, or ERROR, they just get the generic message
    error_detail = str(exc) if LOG_LEVEL == "DEBUG" else "Internal Server Error. Check server logs."
